...
```
这个函数有点长，需要分段来分析。在函数开始处，先检测内存区间的个数，如果小于2，则函数返回-1（对于这个值为啥小于2就是错的，在Linux内核情景分析中有讲述：在PC机中，对最初1M存储空间的使用是特殊的，前*640KB(0~0x9FFFF)*为RAM，从0xA0000开始用于图形显卡，最后的*64KB(0xF0000~0xFFFFF)*则是在EPROM或Flash存储器中的BIOS，因此只要有BIOS存在，内存区间至少有2个）.接下来检测获取到的内存区间大小，如果一个区间的起始地址加上区间长度后反而小了，则说明发生溢出，表示出错了，因此函数退出。然后用一个数组来存储获取到的内存区间的起始地址和结束地址，代码如下
```C
	if (*pnr_map < 2)
		return -1;
